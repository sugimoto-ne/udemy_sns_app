...
新しい機能を実装する際は、以下のルールを必ず守ること：

1. **実装前の要件確認**
   - ユーザーの要件をすべて箇条書きで列挙し、1つも漏らさず実装すること
   - 「本番環境でのみ」「開発環境では無効」などの環境別の動作要件を必ず実装すること
   - 不明点があれば実装前に必ず質問すること

2. **テスト実装**
   - 新しい機能を実装したら対応するテストも必ず書くこと
   - テストコマンドはMakefileを経由して実行すること（`make test-backend`, `make test-e2e`）
   - テスト用コンテナは`api_test`を使うこと（開発用`api`コンテナではない）

3. **テストの並列実行制限**
   - テストの並列実行数はマシン負荷を抑えるため制限すること
     - Go test: `-parallel 2`
     - Playwright: `workers: 1`
   - テストケース内のループは検証に必要な最小回数にとどめること
   - テスト結果やレポートのディレクトリは`.gitignore`に追加すること
...
このセクションでは、システムの品質特性と技術的制約を定義します。すべての開発において、これらの要件を満たすことを意識してください。

| 分類 | 項目 | 要件 | 備考 |
|------|------|------|------|
| **性能** | レスポンスタイム | 一覧取得API：500ms以内 | タイムライン、ユーザー投稿一覧など |
| **性能** | クエリ効率 | N+1クエリの排除 | Preload/Joinsを適切に使用 |
| **運用・保守性** | ログ | アクセスログ・エラーログを構造化して記録 | JSON形式、適切なログレベル |
| **運用・保守性** | エラーハンドリング | ユーザーに適切なエラーメッセージを返す | 内部エラーを露出させない |
| **セキュリティ** | 認証 | HttpOnly Cookie（SameSite=None; Secure）でトークンを管理 | アクセストークン1時間、リフレッシュトークン7日 |
| **セキュリティ** | CORS | フロントエンドのオリジンのみ許可 | 本番環境では適切に設定 |
| **セキュリティ** | 入力検証 | 全入力のバリデーション・サニタイズ | validator/v10使用 |
| **セキュリティ** | セキュリティヘッダー | CSP, X-Frame-Options, HSTS設定 | XSS, クリックジャッキング対策 |
| **セキュリティ** | レートリミット | 認証系API：5回/分、一般API：60回/分 | DDoS対策、ブルートフォース対策 |

### 非機能要件の達成状況

各要件の達成状況については、以下のドキュメントを参照してください:

- **診断レポート**: `docs/NON_FUNCTIONAL_REQUIREMENTS_DIAGNOSTIC.md`
- **性能改善レポート**: `docs/PERFORMANCE_IMPROVEMENT_REPORT.md` (N+1クエリ解消、レスポンスタイム61%改善)
- **セキュリティ改善レポート**: `docs/SECURITY_IMPROVEMENT_REPORT.md` (セキュリティヘッダー、レートリミット実装完了)

### セキュリティ機能の検証

セキュリティヘッダーとレートリミットの動作確認:

```bash
# セキュリティ機能の検証スクリプト
bash backend/scripts/verify_security.sh
```

このスクリプトは以下を確認します:
- 5種類のセキュリティヘッダーが設定されているか
- レートリミットが正しく機能しているか（認証系5回/分、一般60回/分）
- 429エラーレスポンスの形式が正しいか
